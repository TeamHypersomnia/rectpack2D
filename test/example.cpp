...
int main () {
	...
	for(int i = 0; i < RECTS; ++i) {
		rects[i] = rect_xywhf(i, 0, 0, 7+rand()%50,  7+rand()%50);
		ptr_rects[i] = rects+i;
	}
	...
	if(pack(ptr_rects, RECTS, 400, ALLOW_FLIP, bins)) {
		...
		for(int i = 0; i < bins.size(); ++i) {
			...
			for(int r = 0; r < bins[i].rects.size(); ++r) {
				...
				printf("rect %d: index: %d, x: %d, y: %d, w: %d, h: %d, was flipped: %s\n", r, rect->index, rect->x, rect->y, rect->w, rect->h, rect->flipped ? "yes" : " no");
			}
		}
	}
	else {
